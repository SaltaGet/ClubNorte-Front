...
interface DesktopReponerStockTableProps {
  tableData: Product[];
  isLoading: boolean;
...
const DesktopReponerStockTable = ({
  tableData,
  isLoading,
  isLoadingCategories,
  isSearchingByName,
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.display({
        ...
        cell: (info) => (
          <button
            onClick={() => onOpenModal(info.row.original)}
                    ...
                    {row.getVisibleCells().map((cell) => (
                      <td
                        ...
                        className={`px-6 py-4 text-sm text-slate-300 align-top ${
                          cell.column.id === "stock_point_sales"
                            ? ""
                            : "whitespace-nowrap"
                        }`}
                      >
...
export default DesktopReponerStockTable;